	...
	for (int i = 0, k = 0; i < length; i++, k++)
	{
		...
		else
		{
			...
			else
			{
				...
				{
					DrawRectangleRec((Rectangle){rec.x + textOffsetX, rec.y + textOffsetY, glyphWidth, (float)font.baseSize * scaleFactor}, selectBackTint);
...
void DrawPlaylistItem(Font font, std::string musicFileName, Vector2 pos)
{
	// Draw a card
	// item name
	// music duration
	int textPadding = 4;
	Rectangle rec;
	rec.x = pos.x + textPadding;
	rec.y = pos.y + textPadding;
	rec.width = 180;
	rec.height = 80;
	// DrawTextBoxed(font, musicFileName.c_str(), rec, font.baseSize / 1.5f, 1.0f, false, GRAY);
	...
	DrawTextBoxedSelectable(font, musicFileName.c_str(), rec, font.baseSize / 1.5f, 1.0f, false, BLACK, 0, rec.width, BLACK, WHITE);
}
